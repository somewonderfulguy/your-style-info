...
test('menu (navigation) works as expected', async () => {
  ...
  const subLists = Array.from(
    getNav().querySelectorAll('.menuWrapper > ul ul')
  ).map((ul) => ul.parentElement)
  const treeButtons = getNav().querySelectorAll('button')

  const expectedExpandableItems = Object.values(PRIME_ROUTES).filter(
    (val) => val.sub
  )

  const expectedItems: string[] = []
  const findItems = (arr = Object.values(PRIME_ROUTES)) =>
    arr.forEach((route) => {
  ...
  const findDisabled = (arr = Object.values(PRIME_ROUTES)) =>
    arr.forEach((route) => {
